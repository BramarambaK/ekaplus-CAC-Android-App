...
class OnBoardingDotsView(
    ...
    init {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.OnBoardingViewIndicator)
        ...
        try {
            intrinsicWidth = ta.getDimensionPixelSize(R.styleable.OnBoardingViewIndicator_dot_width, -1)
            intrinsicHeight = ta.getDimensionPixelSize(R.styleable.OnBoardingViewIndicator_dot_height, -1)
            intrinsicMargin = ta.getDimensionPixelSize(R.styleable.OnBoardingViewIndicator_dot_margin, -1)
            intrinsicOrientation = ta.getInt(R.styleable.OnBoardingViewIndicator_dots_orientation, -1)
            intrinsicGravity = ta.getInt(R.styleable.OnBoardingViewIndicator_dots_gravity, -1)

            this.animatorResId = ta.getResourceId(
                    R.styleable.OnBoardingViewIndicator_dots_animator,
                    R.anim.scale_with_alpha
            )
            this.animatorReverseResId =
                    ta.getResourceId(R.styleable.OnBoardingViewIndicator_dots_animator_reverse, 0)
            this.backgroundResId = ta.getResourceId(
                    R.styleable.OnBoardingViewIndicator_dot_drawable,
                    R.drawable.onboarding_circle
            )
            this.unselectedBackgroundId = ta.getResourceId(
                    R.styleable.OnBoardingViewIndicator_dot_drawable_unselected,
                    this.backgroundResId
            )
            this.dotTint = ta.getColor(R.styleable.OnBoardingViewIndicator_dot_tint, 0)
        } finally {
            ta.recycle()
        }
        ...
        indicatorBackgroundResId =
                if (this.backgroundResId == 0) R.drawable.onboarding_circle else this.backgroundResId
    ...
    fun setDotDrawable(
        ...
        indicatorBackgroundResId = if (this.backgroundResId == 0) R.drawable.onboarding_circle else this.backgroundResId
