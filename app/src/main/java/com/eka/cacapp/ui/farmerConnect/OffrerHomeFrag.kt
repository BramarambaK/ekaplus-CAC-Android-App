...
import com.eka.cacapp.databinding.OffrerHomeFragBinding
...
import com.eka.cacapp.utils.FarmerConnectUtils.FC_MODE_OFFEROR
...
class OffrerHomeFrag : Fragment() , FilterSelectListener{

    private lateinit var mBinding : OffrerHomeFragBinding
    ...
    private val  PUBLISED_PRICE_TAB ="published_price"
    private val  BIDS_TAB ="bids_tab"
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        mBinding = DataBindingUtil.inflate(inflater, R.layout.offrer_home_frag, container, false)
        ...
        mBinding.serviceTtl.setText(getString(R.string.fc_offr_ttl))
        ...
        mViewModel.listOfBidsOfferorResponse.observe(viewLifecycleOwner, androidx.lifecycle.Observer { it ->
        ...
        mViewModel.farmerConnectGenSettingsResponse.observe(viewLifecycleOwner, androidx.lifecycle.Observer { it ->
            ...
            when (it) {
                is Resource.Success -> {
                    ...
                    AppPreferences.saveValue(Constants.PrefCode.FC_OFFER_TYPE,
                            resultJson.optString("offerType"))
        ...
        mBinding.addOfferBtn.setOnClickListener {

            mViewModel.listOfOffersResponse.removeObserver { mViewModel.listOfOffersResponse }
            findNavController().navigate(R.id.action_offrerHomeFrag_to_createOfferFrag)
    ...
    private fun addBidsToAdapter(listOfData : ListOfOfferData){
        ...
        try {
            ...
            bidListWithStatusAdapter = FcBidsWithStatusAdapter(listOfData,requireContext(),FC_MODE_OFFEROR)
            ...
            bidListWithStatusAdapter.onItemClick = { selectedItem ->
                ...
                findNavController().navigate(R.id.action_offrerHomeFrag_to_bidDetailsFrag)
    ...
    private fun sendListOfBidsReqWithStatus(statusValue:String) {
        ...
        val reqParams= FarmerConnectUtils.getFcReqParamsWithStatus(statusValue,
                userName, FarmerConnectUtils.IN, 8, 0);
        lastSentReqParams = reqParams

        mViewModel.getListOfBidsOfferor(reqParams)
    }
    ...
    private fun showSortBottomSheet() {
        ...
        fcSortAdapter.onItemClick = {
            ...
            if(selectedTab.equals(PUBLISED_PRICE_TAB)){
                ...
                mViewModel.getListOfBidsOfferor(mainObj)
            }
    ...
    private fun getReqParamJson() : JsonObject {
        ...
        val reqParams= FarmerConnectUtils.getFarmerConnectGenReqParamObj(userName, FarmerConnectUtils.IN, 8, 0)
        return reqParams
    ...
    private fun addOffersToAdapter(listOfData : ListOfOfferData){
        ...
        try {
            ...
            bidListAdapter.onItemClick = { selectedItem ->
                ...
                val json = gson.toJson(selectedItem)
                AppPreferences.saveValue(Constants.PrefCode.SELECTED_OFFER,json);
                findNavController().navigate(R.id.action_offrerHomeFrag_to_pulishedPriceDetailsFrag)
    ...
    private fun initSortData(){
            listSorData.clear()
            if(selectedTab.equals(PUBLISED_PRICE_TAB)){
                listSorData = FarmerConnectUtils.getFcSortFields()
            }else {
                listSorData = FarmerConnectUtils.getFcSortFields()
    ...
    override fun onBasicFilterSelected(value: JsonArray, hashMapIsRowCheck : HashMap<String,Boolean>,
        ...
        val filterArr = value
        var mainObj : JsonObject
        if(selectedTab.equals(FarmerConnectUtils.PUBLISED_PRICE_TAB)){
            mainObj = getReqParamJson()
